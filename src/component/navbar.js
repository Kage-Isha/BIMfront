    ...
    <nav className="bg-background w-full flex flex-row items-center justify-between px-4 py-2">
        ...
        <div className="relative md:hidden flex items-center">
          ...
          {toggle && (
            <ul className="absolute w-32 z-10 h-fit bg-[#FFFFFF] shadow-xl top-14 left-0 text-[#7C8DB0] flex flex-col gap-2 items-start p-4 scaleUp">
              <A name="News" href="/news"><li>News</li></A>
              {user && (
                <>
                  <A name="Social" href="/option"><li>Social</li></A>
                  <A name="Donate" href="/contact"><li>Donate</li></A>
                </>
              )}
              <A name="About us" href="/about"><li>About us</li></A>
              <A name="Contact us" href="/contact"><li>Contact us</li></A>
              {!user ? (
                <A name="Sign in" href="/signin"><li>Sign in</li></A>
              ) : (
                <>
                  <div className="flex flex-col items-center">
                    <FaUserAlt className="text-[#605DEC] w-6 h-6" />
                  </div>
                  <B onClick={logout} name="Log out" />
                </>
              )}
            </ul>
          )}
      ...
      <div className="md:flex items-center space-x-8">
        <ul className="hidden md:flex items-center space-x-8 text-[#7C8DB0]">
          ...
          {user && (
            <>
              <A name="Social" href="/"><li>Social</li></A>
              <A name="Donate" href="/"><li>Donate</li></A>
            </>
          )}
          <A name="About us" href="/about"><li>About us</li></A>
          <A name="Contact us" href="/contact"><li>Contact us</li></A>
        ...
        <div className="flex items-center space-x-4">
          {!user ? (
            <Link href="/signup"><B name="Sign up" /></Link>
          ) : (
            <B onClick={logout} name="Logout" />
          )}
        </div>
      </div>
    </nav>
